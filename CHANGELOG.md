...
- `plexuspact_engine::pushdown` evaluates a contract inside a warehouse instead of pulling its rows: `compile` turns the contract into one aggregate scan (row count, null and distinct counts, bounds, one `SUM(CASE …)` per row-level check, the column profile) plus one statement per `custom_expr`/`assert`, in Postgres/Redshift, Snowflake, Databricks or BigQuery SQL; `follow_ups` fetches failure samples, duplicated keys, small value sets and the keys `primary_key`/`references` need; `decode` returns the same `EngineOutput` — same check ids, parameters, observed statistics and messages — the streaming engine produces. `plexuspact_core::assemble` maps that into a `RunResult`, so a warehouse run and a file run compare alike. Where the warehouse cannot do what the check asks (a number bound on a JSON column, a timestamp from Postgres text) the check reports that it could not run rather than passing; the guide "Checking a warehouse table in place" lists every such difference.
